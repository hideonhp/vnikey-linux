...
pub fn find_tone_target_index(chars: &[char]) -> Option<usize> {
    ...
    // Check `gi` exception
    if start > 0 && chars[start] == 'i' && chars[start - 1] == 'g' {
    ...
    let target_offset = match vowel_count {
        ...
        3 => {
            // Check `uye` exception
            let v1 = get_base_vowel_and_tone(chars[start]).0;
            let v2 = get_base_vowel_and_tone(chars[start + 1]).0;
            let v3 = get_base_vowel_and_tone(chars[start + 2]).0;
            if v1 == 'u' && v2 == 'y' && (v3 == 'e' || v3 == 'ê') {
                2
            } else {
                1
            }
...
mod tests {
    ...
    fn test_tone_target_index() {
        ...
        assert_eq!(find_tone_target_index(&['m', 'a', 'i']), Some(1)); // 'a' and 'i' are 2 vowels, wait, mai is 2 vowels!

        // Let's check mai
        // 'm', 'a', 'i' -> no coda -> index 0 of cluster -> 'a' -> Some(1)
        assert_eq!(find_tone_target_index(&['m', 'a', 'i']), Some(1));

        // 2 vowels, no coda -> 1st vowel
        assert_eq!(find_tone_target_index(&['h', 'o', 'a']), Some(1)); // 'o'
        assert_eq!(find_tone_target_index(&['t', 'h', 'u', 'y']), Some(2)); // 'u'

        // 2 vowels, with coda -> 2nd vowel
        assert_eq!(find_tone_target_index(&['h', 'o', 'a', 'n', 'g']), Some(2)); // 'a'

        // 3 vowels -> 2nd vowel
        assert_eq!(find_tone_target_index(&['o', 'a', 'i']), Some(1)); // 'a'
        assert_eq!(find_tone_target_index(&['k', 'h', 'u', 'y', 'a']), Some(3)); // 'y'

        // uye exception
        assert_eq!(find_tone_target_index(&['n', 'g', 'u', 'y', 'ê', 'n']), Some(4)); // 'ê'

        // qu exception
        assert_eq!(find_tone_target_index(&['q', 'u', 'a']), Some(2)); // 'a'
        assert_eq!(find_tone_target_index(&['q', 'u', 'a', 'n']), Some(2)); // 'a'
        assert_eq!(find_tone_target_index(&['q', 'u', 'a', 'i']), Some(2)); // 'a'
        assert_eq!(find_tone_target_index(&['q', 'u', 'y']), Some(2)); // 'y'

        // gi exception
        assert_eq!(find_tone_target_index(&['g', 'i', 'a']), Some(2)); // 'a'
        assert_eq!(find_tone_target_index(&['g', 'i', 'e', 'n', 'g']), Some(2)); // 'e'
        assert_eq!(find_tone_target_index(&['g', 'i']), Some(1)); // 'i'
    }
}
INNER_EOF
