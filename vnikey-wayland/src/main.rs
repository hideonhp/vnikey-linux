                    ...
                    if let Some(xkb_state) = state.xkb_state.as_ref() {
                        ...
                        if !cycle_key.is_empty() {
                            let mod_match = if cycle_mod.is_empty() {
                                ...
                                (has_ctrl
                                    && (cycle_mod.contains("control")
                                        || "control".contains(cycle_mod)))
                                    || (has_shift
                                        && (cycle_mod.contains("shift")
                                            || "shift".contains(cycle_mod)))
                                    || (has_alt
                                        && (cycle_mod.contains("alt") || "alt".contains(cycle_mod)))
                                    || (has_super
                                        && (cycle_mod.contains("super")
                                            || "super".contains(cycle_mod)))
                            };
                            let key_match = key_name.eq_ignore_ascii_case(cycle_key)
                                || (key_name.len() >= cycle_key.len()
                                    && key_name.as_bytes().windows(cycle_key.len()).any(
                                        |window| window.eq_ignore_ascii_case(cycle_key.as_bytes()),
                                    ));
